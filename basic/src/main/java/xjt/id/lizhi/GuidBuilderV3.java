...
public class GuidBuilderV3 {
    ...
    public GuidBuilderV3(int serverId) {
        Date old = baseTime();
    ...
    protected long nextId(int type) {
        ...
        if (serverFlag() < 1024) {
            ...
            int serverIdTwoHighBit = serverFlag() >> 10;
            int serverIdSevenLowBit = serverFlag() & 1023;
            return (((VERSION << 30 | timeFlag()) << 10 | serverIdSevenLowBit) << 12 | counter()) << 9
                    | serverIdTwoHighBit << 7 | type;
        }
    }
    ...
    protected long nextId(ModuleType type) {
        ...
        if (serverFlag() < 1024) {
            ...
            int serverIdSevenLowBit = serverFlag() & 1023;
            return (((VERSION << 30 | timeFlag()) << 10 | serverIdSevenLowBit) << 12 | counter()) << 9
                    | serverIdTwoHighBit << 7 | type.getRaw();
        }
    }
    ...
    protected long nextId(ModuleType type, Date date) {
        ...
        if (serverFlag() < 1024) {
            ...
            int serverIdTwoHighBit = serverFlag() >> 10;
            int serverIdSevenLowBit = serverFlag() & 1023;
            return (((VERSION << 30 | timeFlag(date)) << 10 | serverIdSevenLowBit) << 12 | counter()) << 9
                    | serverIdTwoHighBit << 7 | type.getRaw();
        }
    }
    ...
    public int extractServerId(long id) {
        int serverIdTwoHighBit = (int) (id >> 7) & 3;
        if (serverIdTwoHighBit == 0) {
            return (int) (id << 33 >>> 54);
        } else {
            return (int) ((serverIdTwoHighBit << 10) | (id << 33 >>> 54));
        }
    }
